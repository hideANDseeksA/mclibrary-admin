...
const BookActivityTable = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('https://backend-j2o4.onrender.com/api/bookactivities');
  ...
  const handleAction = async (activityId, actionType) => {
    ...
    if (result.isConfirmed) {
      ...
      try {
        const response = await fetch(`https://backend-j2o4.onrender.com/api/bookactivity_reserve/${activityId}`, {
          method: 'PUT',
          headers: {
        ...
        const updatedResponse = await fetch('https://backend-j2o4.onrender.com/api/bookactivities');
